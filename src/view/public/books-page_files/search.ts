...
var pathNameUrl: string | string[] | undefined;
...
let getPathUrl = (pathNameUrl: string[] | string | undefined) => {
...
var callbackQueryMiniItemsSearch = function (res: ResponseWithData, text: string) {
    $('.loader').show();
...
var msgResultSearchText = function (text: string, number_found: number) {
...
var callbackQueryItemsSearch = function (res: ResponseWithData, text: string) {
...
var requestBooksSearch = function (callback: ((res: ResponseWithData, text: string) => void)) {
    var text = htmlspecialchars(String($('#search').val()));
    if (text.length > 0) {
        text = text.replace(/(^\s+|\s+$)/g, '');
        var textEncode = encodeURIComponent(text); // shielding request

        doAjaxQuery('GET', '' + pathUrl + '/api/v1/books?search=' + textEncode + '', {},
            function (res) {
                callback(res, text);
            });
    } else {
...
$("#search").on("keypress", function (event) {
    ...
    if (event.code === "Enter") {
        ...
        if (!!text && text.length > 0) {
            ...
            if (pathUrl == '/admin') {
                requestBooksSearch(function (res: ResponseWithData) {
 ...
 */
$(function () {  // might be problem document).ready
    (function () {
        ...
        if (!!search) {
            ...
            var search_text: string | string[] | undefined | null
            if (!!location) {
                search_text = search;
                if (!!search_text) {
            ...
            let search_string: string = decodeURIComponent((!search_text) ? ' ' : search_text[1]);
